...
import io.quarkus.grpc.GrpcService;
  ...
  @GrpcService
  private static class AdQuarkusServiceImpl extends hipstershop.AdQuarkusServiceGrpc.AdQuarkusServiceImplBase {
     ...
     */
    @Override
    public void getAdsQuarkus(AdQuarkusRequest req, StreamObserver<AdQuarkusResponse> responseObserver) {
      ...
      try {
        ...
        if (req.getContextKeysCount() > 0) {
          for (int i = 0; i < req.getContextKeysCount(); i++) {
            Collection<Ad> ads = this.getAdsByCategory(req.getContextKeys(i));
            allAds.addAll(ads);
          }
        } else {
        ...
        AdQuarkusResponse reply = AdQuarkusResponse.newBuilder().addAllAds(allAds).build();
