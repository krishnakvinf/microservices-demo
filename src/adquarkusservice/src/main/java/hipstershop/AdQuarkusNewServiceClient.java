...
public class AdQuarkusServiceClient {

  private static final Logger logger = LogManager.getLogger(AdQuarkusServiceClient.class);

  private final ManagedChannel channel;
  private final hipstershop.AdQuarkusServiceGrpc.AdQuarkusServiceBlockingStub blockingStub;

  /** Construct client connecting to Ad Service at {@code host:port}. */
  private AdQuarkusServiceClient(String host, int port) {
    this(
  ...
  private AdQuarkusServiceClient(ManagedChannel channel) {
    this.channel = channel;
    blockingStub = hipstershop.AdQuarkusServiceGrpc.newBlockingStub(channel);
  }
  ...
  /** Get Ads from Server. */
  public void getAdsQuarkus(String contextKey) {
    logger.info("Get Ads with context " + contextKey + " ...");
    AdQuarkusRequest request = AdQuarkusRequest.newBuilder().addContextKeys(contextKey).build();
    AdQuarkusResponse response;

    try {
      response = blockingStub.getAdsQuarkus(request);
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    AdQuarkusServiceClient client = new AdQuarkusServiceClient(host, serverPort);
    try {
      client.getAdsQuarkus(contextKeys);
    } finally {
      client.shutdown();
    }

    logger.info("Exiting AdQuarkusServiceClient...");
  }
}
